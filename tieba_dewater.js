...
function extract_floor_info(bot) {
    ...
    re["time"] = info.find('ul.p_tail li').eq(1).text() || info.find('.tail-info').eq(-1).text();
    re["id"] = info.find('ul.p_tail li').eq(0).text().replace(/楼.*$/,'') || info.find('.tail-info').eq(-2).text().replace(/楼.*$/,'');
